...
Class Reservation{
    ...
    public function dateReservations(){
        return "<strong>Hotel: ".$this->hotel->getNom()."</strong> / Chambre : ".$this->chambre->getNumChambre()." (".$this->chambre->getNbLits()." lits - ".$this->chambre->getPrix()." € - Wifi:".$this->chambre->getWifi().") du ".$this->dateEntree->format('d/m/Y')." au ".$this->dateSortie->format("d/m/Y");
    } 
    // Synthèse de la réservation
    public function syntheseReservations(){
        return 
    ...
    public function __toString(){
        return 
        " du ".$this->dateEntree->format('d/m/Y').
        " au ".$this->dateSortie->format('d/m/Y').")";
    }
}
